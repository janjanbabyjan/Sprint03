...
test('TC03', async ({ page }) => {
  ...
  test('TC25', async ({ page }) => {
    ...
    const CheckTH5 = await expect(page.getByText('Ada 1 item di keranjang Anda.')).toBeVisible()
  ...
  test('TC30', async ({ page }) => {
    ...
    const CheckTHNN1 = await expect(page.getByRole('link', { name: 'เครื่องเขียน' })).toBeVisible()
    const CheckTHNN2 = await expect(page.getByText('ผู้จัดหาสินค้า', { exact: true })).toBeVisible()
    const CheckTHNN3 = await expect(page.getByText('ไม่มีผู้จัดหาสินค้า')).toBeVisible()
    const CheckTHNN4 = await expect(page.getByRole('link', { name: 'แบรนด์' })).toBeVisible()
  });
